...
setwd("D:/01_Digea/02_Proj/02_Proj_CP/01 Fase I/09_4_Etapa/01_VERIF_INSTR_PER_2012_2021")
...
?agnes
DEN_S_2012=agnes(FOR_CUST_2012[,1:2], method ="single") #Aplicar a f_unção agnes aos dados de força de trabalho e custeio total de cada exercício, usando o método Single; 
DEN_W_2012=agnes(FOR_CUST_2012[,1:2], method ="ward") #Aplicar a função agnes aos dados de força de trabalho e custeio total de cada exercício, usando o método Ward;
...
MAPA_2012$categoria <- cut(MAPA_2012$cod_cluster, breaks = c(0, 1, 2, 3, 4),labels =  c("Micro", "Pequeno", "Médio", "Grande")) #Transformar em categorias os conglomerados.
str(MAPA_2012)
...
ggplot(MAPA_2012)+
 geom_sf(aes(fill=categoria), colour = "black", size = 0.1)+
  ...
  #scale_fill_gradient(low = "green", high = "blue", name = "Conglomerados")+
  ...
  #scale_fill_distiller(palette = "Blues", name="Conglomerados", limits = NULL) +
  xlab("") +  ylab("") +geom_sf_label(aes(label = abbrev_state),label.padding = unit(0.5, "mm"),size = 3)+ 
  ...
  #ggtitle("Força Trab. Inst. x Custeio Total", subtitle = "2012")+
  ...
  geom_image(aes(x=-37, y=-27), #Inserir imagem.
             image="D:/01_Trabalhos Digea/02_Demandas/02_ Dados para o projeto Capacidade Produtiva da RBMLQ-I/09_4_Etapa/01_VERIF_INSTR_PER_2012_2021/Mapas/Marca Inmetro/vertical-preta-traco.png",
                   size=0.15)+
  labs(title = "Força Trab. Inst. x Custeio Total", #Inserir título e subtítulo.
       subtitle = "2012",
