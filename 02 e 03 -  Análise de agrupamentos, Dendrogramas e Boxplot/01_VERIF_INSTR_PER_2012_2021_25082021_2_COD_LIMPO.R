...
bdcp_2012_2021 <- read_excel("D:/01_Trabalhos Digea/02_Demandas/02_ Dados para o projeto Capacidade Produtiva da RBMLQ-I/01_BDCP_Estrutura_INST_PER_2012_2021.xlsx", sheet=1, col_names=TRUE) #fazer a leitura/importacao automática do arquivo (xlsx) BDCP_Estrutura de acordo com a atividade que se queira analisar. fonte de pesquisa (https://www.luisotavio.pro/blog/como-ler-planilha-excel-no-r/)
...
write.table(medias, file ="medias_2012_2019.csv", sep=";", col.names = TRUE, row.names = FALSE) #Exportar dados geográficos para leitura em Excel
...
MAPA_2012_2019$categoria <- cut(MAPA_2012_2019$cod_cluster_modificado, breaks = c(0, 1, 2, 3, 4),
                                labels =  c("Grande Eficiência",
                                            "Média Eficiência",
                                            "Pequena Eficiência",
                                            "Micro Eficiência")) #Transformar em categorias os conglomerados.

classe <- c("Grande Eficiência" = "#0570b0",
            "Média Eficiência" = "#74a9cf",
            "Pequena Eficiência" = "#fd8d3c", #Definição da cor das classes
            "Micro Eficiência" = "#e31a1c")
...
ggplot(MAPA_2012_2019)+
  ...
  geom_image(aes(x=-37, y=-27), #Inserir imagem.
             ...
             size=0.15)+
  labs(title = "Força de Trabalho de  Instrumentos Periódicos. x Custeio Total - Função kmeans", #Inserir título e subtítulo.
...
classe <- c("Grande Eficiência" = "#0570b0",#Definição da cor das classes
            "Média Eficiência" = "#74a9cf",
            "Pequena Eficiência" = "#fd8d3c", 
            "Micro Eficiência" = "#e31a1c")
